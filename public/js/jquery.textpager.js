    ...
    e.fn.textpager = function(options) {
        ...
        if(textareaHeight<fulltextHeight){
            ...
            if (eOptions.controlArrows==='') {
                ...
                $('<a>').addClass('icon-back-01').addClass('unactive').html('<span><</span>').appendTo($(eOptions.controlArrows));
                $('<a>').addClass('icon-fwrd-01').html('<span>></span>').appendTo($(eOptions.controlArrows));
            }
            ...
            var self = this;
            $(eOptions.controlArrows).find('.icon-back-01').unbind('click').click(function(){
                ...
                $(eOptions.controlArrows).find('.icon-fwrd-01').removeClass('unactive');
                var nextPage = (thisPage-pageNow)*textareaHeight;
                margTop -= nextPage;
                pageNow = thisPage;
               controlElements = $('<'+eOptions.controlPagesContent+'>')
                        .attr('data-page','')
                        .html('<span> '+thisPage+' de '+pages+'</span>')
                        .addClass('tp-page')
                        .addClass(!1? 'active' : '')
                        .prop("outerHTML");
            $(eOptions.controlPages).html(controlElements);self.animateStep(self, textareaWidth, margTop, false);                
            });

            $(eOptions.controlArrows).find('.icon-fwrd-01').unbind('click').click(function(){
                var thisPage = pageNow+1;
                if(thisPage>pages){
                    return;
                }
                ...
                $(eOptions.controlArrows).find('.icon-back-01').removeClass('unactive');
                var nextPage = (thisPage-pageNow)*textareaHeight;
                margTop -= nextPage;
                pageNow = thisPage;
              controlElements = $('<'+eOptions.controlPagesContent+'>')
            ...
            $(eOptions.controlPages).find(eOptions.controlPagesContent+'.tp-page').unbind('click').click(function(){
                ...
                if (thisPage==1) {
                    $(eOptions.controlArrows).find('.icon-back-01').addClass('unactive');
                    $(eOptions.controlArrows).find('.icon-fwrd-01').removeClass('unactive');
                } else if(thisPage==pages) {
                    $(eOptions.controlArrows).find('.icon-fwrd-01').addClass('unactive');
                    $(eOptions.controlArrows).find('.icon-back-01').removeClass('unactive');
                } else {
                    $(eOptions.controlArrows).find('.icon-fwrd-01').removeClass('unactive');
                    $(eOptions.controlArrows).find('.icon-back-01').removeClass('unactive');
