...
class Empresa extends Model {
     ...
     * @var array
     */
    protected $fillable = ['titulo', 'descripcion', 'clasificacion', 'cuerpo', 'foto', 'foto2', 'foto3', 'foto4', 'foto5'];

    public function setFotoAttribute($foto) {
        if ($foto == 'null') {
            /* if (!is_null(\DB::table('empresas')->find($this->id))) {
              \Storage::disk('local')->delete(\DB::table('empresas')->find($this->id)->foto);
              }
    ...
    public function setFoto2Attribute($foto2) {
        if ($foto2 == 'null') {
            /* if (!is_null(\DB::table('empresas')->find($this->id))) {
              \Storage::disk('local')->delete(\DB::table('empresas')->find($this->id)->foto2);
    ...
    public function setFoto3Attribute($foto3) {
        if ($foto3 == 'null') {
            /* if(!is_null(\DB::table('empresas')->find($this->id))){
              \Storage::disk('local')->delete(\DB::table('empresas')->find($this->id)->foto3);
    ...
    public function setFoto4Attribute($foto4) {
        if ($foto4 == 'null') {
            /* if(!is_null(\DB::table('empresas')->find($this->id))){
              \Storage::disk('local')->delete(\DB::table('empresas')->find($this->id)->foto4);
    ...
    public function setFoto5Attribute($foto5) {
        if ($foto5 == 'null') {
            /* if(!is_null(\DB::table('empresas')->find($this->id))){
              \Storage::disk('local')->delete(\DB::table('empresas')->find($this->id)->foto5);
              }
